...
(function (factory) {
   ...
   * @constructor
   */
  var Console = function (commands, option) {
  ...
  Console.prototype.dispatch = function (command) {
    ...
    var cmd = this.cfg.caseSensitive ? parts[0] : parts[0].toLowerCase()
    var handlerObj = this.commands[cmd]
    var result
    if (handlerObj) {
      // support alias

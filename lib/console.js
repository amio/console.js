...
(function () {
  ...
  function createInputListener(cnsl) {
    ...
    return function (e) {
      ...
      case 38:
        // Up Arrow
        cnsl.history[i + 1] !== void 0 && (e.target.value = cnsl.history[++i]);
        i >= cnsl.history.length && (i = cnsl.history.length - 1);
        e.preventDefault();
        break;
      case 40:
        // Down Arrow
        cnsl.history[i] !== '' && (e.target.value = cnsl.history[--i]);
        i < 0 && (i = 0);
        break;
      case 9:
        // Tab
        i = 0;
        cnsl.history[0] = e.target.value = cnsl.autoComplete();
        e.preventDefault();
        break;
      default:
        i = 0;
  ...
  Console.prototype.printHelp = function (filterFn) {
    ...
    for (i = 0; i < names.length; i++) {
      ...
      lineout = ''
      if (cmd.usage) lineout += (' ' + cmd.usage)
      if (cmd.desc) lineout += (' ' + cmd.desc)
      this.log(lineout);
    }
  }
